...
\begin{frame}
	...
	\begin{itemize}
		...
		\item  Let $\Lambda = \operatorname{diag}\big(\boldsymbol{c}(t\!+\!1)\big)$ be a positive semi-definite diagonal weighting matrix reflecting the electricity tariff structure
...
\begin{frame}
	...
	\begin{itemize}
		...
		\end{IEEEeqnarray*}
		where $\Lambda = \operatorname{diag}\big([\boldsymbol{c}(t\!+\!1)^{T}, \boldsymbol{c}(t\!+\!2)^{T}, \ldots, \boldsymbol{c}(t\!+\!n)^{T}]\big)$
...
%			\item  Operational peak demand is much higher for multi-period control horizon
%		\end{itemize}
%		
%		\item  MIQP:
%		\begin{itemize}
...
%			\item  Operational peak demand is markedly lower for multi-period control horizon, while single-period control horizon results in fewer charge/discharge cycles
%		\end{itemize}
%		
%		\item  MILP solves faster resulting in shorter simulation runtime than MIQP
%		
%		\item  MIQP has much lower operational peak demand than MILP\footnote{\tiny QP penalises large power imports from, and exports to, the grid during a given time interval disproportionately more heavily than small imports/ exports.  LP penalises large power imports from the grid during a given time interval proportionately equally as small imports.  Therefore, QP tends to charge the battery in anticipation of supplying excess load (i.e., load in excess of rooftop PV generation) during peak times by importing relatively small quantities of power from the grid over multiple time intervals.  As a consequence operational maximum demand tends to be much lower under QP than LP}
%	
%	\end{itemize}
%
%\end{frame}
